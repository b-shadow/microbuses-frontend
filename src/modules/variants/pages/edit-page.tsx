...
import {
  ArrowLeft, GitBranch, Map as MapIcon, AlignLeft, Loader2, CheckCircle2, XCircle, X
} from 'lucide-react'

import { getVariant, updateVariant, listLinesLite, LineRowLite } from '../services/module.service'
...
export function VariantsEditPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (vRes.success) {
        setLineId(String(vRes.data.line_id))
        setName(vRes.data.name)
        setDescription(vRes.data.description ?? '')
        setActive(vRes.data.is_active)
      } else {
        showToast(vRes.message || 'Error al cargar', 'error')
      }
  ...
  if (loadingData) return <div className="flex items-center justify-center py-20"><Loader2 className="h-6 w-6 animate-spin text-fuchsia-500" /></div>
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-50">Editar variante</h1>
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">Actualizar la informacion de la ruta.</p>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="max-w-2xl space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div>
          ...
          <div className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 px-4 py-2.5 dark:border-slate-700 dark:bg-slate-800/50">
            ...
            <div className="flex items-center gap-2">
               ...
               <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">{selectedLine?.nombre_linea || lineId}</span>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative"><GitBranch className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input type="text" required value={name} onChange={(e) => setName(e.target.value)} className={inputCls} /></div>
        </div>

        <div>
          <label className={labelCls}>Descripcion</label>
          <div className="relative"><AlignLeft className="absolute left-3 top-3 h-4 w-4 text-slate-400" /><textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className={`${inputCls} resize-none pt-2.5`} /></div>
        </div>
        ...
        <div>
          ...
          <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-3 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800/50">
            <div className="relative flex items-center">
              ...
              <div className="h-6 w-11 rounded-full bg-slate-200 transition-colors peer-checked:bg-fuchsia-500 dark:bg-slate-700"></div>
              <div className="absolute left-1 top-1 h-4 w-4 rounded-full bg-white transition-transform peer-checked:translate-x-5"></div>
            </div>
            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{active ? 'Variante Activa' : 'Variante Inactiva'}</span>
          </label>
        </div>
        ...
        <div className="flex justify-end gap-2 border-t border-slate-100 pt-5 dark:border-slate-800">
          ...
          <button type="submit" disabled={loading} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-fuchsia-500 to-fuchsia-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:from-fuchsia-600 hover:to-fuchsia-700 disabled:opacity-50">
