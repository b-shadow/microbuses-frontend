...
import {
  GitBranch, Plus, Search, X, Loader2, CheckCircle2, XCircle,
  MoreVertical, ChevronDown, Eye, Pencil, Trash2, Activity, Map as MapIcon, AlignLeft
} from 'lucide-react'

import { deleteVariant, listLinesLite, listVariants, LineRowLite, VariantRow } from '../services/module.service'
...
function ActionDropdown({ variant, onDeleteClick }: {
  variant: VariantRow; onDeleteClick: () => void
}) {
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full z-40 mt-1 w-48 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-xl dark:border-slate-700 dark:bg-slate-900">
            <button onClick={() => { navigate(`/variants/${variant.id}`); setOpen(false) }} className="flex w-full items-center gap-2.5 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-800">
              <Eye className="h-3.5 w-3.5" /> Ver detalle
            </button>
            <button onClick={() => { navigate(`/variants/${variant.id}/edit`); setOpen(false) }} className="flex w-full items-center gap-2.5 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-800">
...
export function VariantsPage() {
  ...
  const load = async () => {
    setLoading(true)
    const [vRes, lRes] = await Promise.all([listVariants(), listLinesLite()])
    if (vRes.success) setRows(vRes.data)
    else showToast(vRes.message || 'Error al cargar variantes', 'error')
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((d) => {
      const line = lineMap.get(String(d.line_id))
  ...
  const executeDelete = async () => {
    ...
    const res = await deleteVariant(deleteTarget.id)
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-50">Variantes de Ruta</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Gestion de las subrutas o variantes asociadas a las lineas.</p>
        </div>
        <Link to="/variants/create" className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-fuchsia-500 to-fuchsia-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:from-fuchsia-600 hover:to-fuchsia-700 hover:shadow-md active:scale-[0.98]">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="inline-flex flex-1 min-w-[200px] relative">
          ...
          <input type="text" placeholder="Buscar por nombre de variante o linea..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm placeholder:text-slate-400 focus:border-fuchsia-400 focus:outline-none focus:ring-2 focus:ring-fuchsia-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:focus:border-fuchsia-600 dark:focus:ring-fuchsia-900/40" />
        ...
        <div className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <MapIcon className="h-4 w-4 text-slate-500" /><span className="text-sm text-slate-600 dark:text-slate-400">Total</span><span className="text-sm font-bold text-slate-900 dark:text-slate-100">{rows.length}</span>
        </div>
      ...
      {loading && <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-fuchsia-500" /><span className="ml-2 text-sm text-slate-500">Cargando variantes...</span></div>}
      ...
      {!loading && (
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-800/30">
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Variante</th>
                    ...
                    <tr key={d.id} className="transition-colors hover:bg-slate-50/70 dark:hover:bg-slate-800/40">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-100 to-fuchsia-200 text-fuchsia-700 dark:from-fuchsia-900/50 dark:to-fuchsia-800/50 dark:text-fuchsia-300">
                            <GitBranch className="h-5 w-5" />
                          </div>
                          <div className="min-w-0">
                            <p className="truncate text-sm font-bold text-slate-800 dark:text-slate-200">{d.name}</p>
                            <p className="truncate text-xs text-slate-500 dark:text-slate-400">{d.description || 'Sin descripcion'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-slate-700 dark:text-slate-300">{line?.nombre_linea || `ID: ${d.line_id}`}</span>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4 text-right">
                        <ActionDropdown variant={d} onDeleteClick={() => setDeleteTarget(d)} />
                      </td>
                    </tr>
                  )
                })}
                {filteredRows.length === 0 && (
                  <tr><td colSpan={4} className="px-5 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">No se encontraron variantes.</p>
                    </div>
                  </td></tr>
                )}
              </tbody>
          ...
          <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/50 px-5 py-3 dark:border-slate-800 dark:bg-slate-800/30">
            <span className="text-xs text-slate-500 dark:text-slate-400">{filteredRows.length} de {rows.length} variantes</span>
          </div>
        </div>
      )}

      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={executeDelete} loading={deleteLoading} title="Eliminar Variante" description={`Estas seguro que deseas eliminar la variante "${deleteTarget?.name}"? Esta accion no se puede deshacer.`} />
    </section>
  )
}
